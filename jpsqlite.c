...
static int copy_sqlite(void) {
	...
	AddressList *a;
	CalendarEventList *c;
	char begin[32], end[32], repeatEnd[32], *pRepeatEnd, due[32], date[32];
	...
	for (; a; a=a->next) {
		...
		CHK(sqlite3_bind_text(dbpstmt,25,a->maddr.addr.entry[entryCustom2],-1,SQLITE_STATIC),"AB24")
	...
	CHK(sqlite3_prepare_v2(db,
		...
		"    RepeatDaySa, Description, Note "		// 20
		") values ("
	...
	for (; c; c=c->next) {
		...
		CHK(sqlite3_bind_int(dbpstmt,10,1 - (c->mcale.cale.repeatForever & 0x01)),"DB10")
		...
		CHK(sqlite3_bind_text(dbpstmt,21,c->mcale.cale.description,-1,SQLITE_STATIC),"DB21")
		CHK(sqlite3_bind_text(dbpstmt,22,c->mcale.cale.note,-1,SQLITE_STATIC),"DB22")
		CHKDONE(sqlite3_step(dbpstmt),"DST")
		CHK(sqlite3_clear_bindings(dbpstmt),"DCL")
		CHK(sqlite3_reset(dbpstmt),"DRST")
		if (nrecsD > 30000) break;
	}
	...
	CHK(sqlite3_prepare_v2(db,
		"insert into MemoCategory (Id, Label) "
		"values (:Id, :Label)", -1, &dbpstmt, NULL), "MCPRE")
	for (i=0; i<16; ++i) {
	...
	for (i=0; i<16; ++i) {
		...
		CHK(sqlite3_clear_bindings(dbpstmt),"ECCL")
		CHK(sqlite3_reset(dbpstmt),"ECRST")
	}
	CHK(sqlite3_finalize(dbpstmt),"ECFIN")
	...
	for (; explst; explst=explst->next) {
		...
		CHK(sqlite3_bind_int(dbpstmt,4,e.type),"EB4")
		CHK(sqlite3_bind_int(dbpstmt,5,e.payment),"EB5")
		CHK(sqlite3_bind_int(dbpstmt,6,e.currency),"EB6")
		CHK(sqlite3_bind_text(dbpstmt,7,e.amount,-1,SQLITE_STATIC),"EB7")
		CHK(sqlite3_bind_text(dbpstmt,8,e.vendor,-1,SQLITE_STATIC),"EB8")
		CHK(sqlite3_bind_text(dbpstmt,9,e.city,-1,SQLITE_STATIC),"EB9")
		CHK(sqlite3_bind_text(dbpstmt,10,e.attendees,-1,SQLITE_STATIC),"EB10")
		CHK(sqlite3_bind_text(dbpstmt,11,e.note,-1,SQLITE_STATIC),"EB11")
	...
	jp_logf(JP_LOG_GUI,"Finished SQLite3 transaction:\n"
		...
		, nrecsT, nrecsTC, nrecsM, nrecsMC, nrecsE, nrecsEC
	);
